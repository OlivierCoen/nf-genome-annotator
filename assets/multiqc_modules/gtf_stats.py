...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self):
        ...
        # Find and load any stat file
        self.mrna_data = {}
        self.transcript_data = {}
        for f in self.find_log_files("gtf_stats"):
            self.parse_stats(f)

        if len(self.quast_data) == 0:
            raise ModuleNoSamplesFound

        logger.info(f"Found {len(self.quast_data)} reports")

        # Write parsed report data to a file
        self.write_data_file(self.quast_data, "multiqc_quast")

        # Basic Stats Table
        self.populate_general_stats_table()
    ...
    def populate_general_stats_table(self):
